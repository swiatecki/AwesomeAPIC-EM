...
def versionChecker():
	...
	controller_url = "https://173.38.218.144"
	print("dick")
	# Get Devices
	...
	for item in switches:
		...
		line["id"] = "Switch " + item["family"]
		line["comment"] = " software = " + item["softwareVersion"]+ "," +" Family Duplicates = %d, Duplicate Software = %d, Newer Software = %d, Older Software = %d" % (item["family_duplicates"], item["software_duplicates_equal"], item["software_duplicates_newer"], item["software_duplicates_older"])
	...
	for item in routers:
		...
		if item["software_duplicates_newer"] > 0:
			...
			line["score"] = 10
		#print ("Family = " + item["family"] + " software = " + item["softwareVersion"]+" Family Duplicates = %d\nDuplicate Software = %d Newer Software = %d Older Software = %d\n" % (item["family_duplicates"], item["software_duplicates_equal"], item["software_duplicates_newer"], item["software_duplicates_older"]))
	result["totalScore"] = ((len(switches)+len(routers))-badscore)/(len(switches)+len(routers))*10
